        ...
        public class Handler : IRequestHandler<Command, ResponseObject>
        {
            ...
            public async Task<ResponseObject> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    await contractRepository.InsertRecordAsync(request.Adapt<Contract>());
                ...
                Log.Information($"Returning response HTTP {response.ResponseCode} from InsertContract command", response);
                return response;
            }
        }
    }
